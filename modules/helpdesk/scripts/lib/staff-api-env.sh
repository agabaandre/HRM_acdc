...
helpdesk_inherit_sensitive_from_portal_env() {
    ...
    local apm_env="$staff_root/modules/apm/.env"
    ...
    for key in STAFF_API_USERNAME STAFF_API_PASSWORD STAFF_API_TOKEN; do
        ...
        if ! dotenv_value_present "$val"; then
            if [[ "$key" == "STAFF_API_TOKEN" ]]; then
                printf 'error: %s is not set in %s — add it to %s/modules/apm/.env or ensure APM vendor is installed so setup can resolve config(services.staff_api.token).\n' \
                    "$key" "$backend_env" "$staff_root" >&2
            else
                printf 'error: %s is not set in %s — add it to %s/modules/apm/.env (same values as `php artisan staff:sync`).\n' \
                    "$key" "$backend_env" "$staff_root" >&2
            fi
            return 1
        fi
