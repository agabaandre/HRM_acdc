/**
 * Non-travel memos index — Vue 3 + Vuetify 3
 */
(function () {
    'use strict';

    const MOUNT_ID = 'non-travel-index-app';
    ...
    function bootNonTravelIndex(mountEl, cfg) {
        if (!mountEl || !cfg) {
            ...
            setup() {
                ...
                const exportUrl = computed(() => {
                    if (activeTab.value === 'mySubmitted' && cfg.routes?.exportMySubmitted) {
                        return buildExportUrl(cfg.routes.exportMySubmitted, filters.value);
                    }
                    if (activeTab.value === 'allMemos' && cfg.routes?.exportAll) {
                        return buildExportUrl(cfg.routes.exportAll, filters.value);
                    }
                    return null;
                });
                ...
                const headers = computed(() => {
                    ...
                    base.push(
                        ...
                        { title: 'Date', key: 'memo_date', sortable: false, width: 110 },
                ...
                async function loadItems() {
                    ...
                    try {
                        ...
                        if (!res.ok) {
                            throw new Error(data.error || 'Could not load non-travel memos.');
                        }
                        ...
                        notify(e.message || 'Could not load non-travel memos.');
                    } finally {
                        loading.value = false;
                    }
                }
                ...
                function confirmDelete(item) {
                    ...
                    if (!window.confirm('Delete this non-travel memo? This cannot be undone.')) return;
...
<v-app class="nt-vuetify-app" theme="apmLight">
  <v-container fluid class="pa-0">
      ...
      <v-card-text class="pt-4">
        ...
        >
          ...
          <template #item.title="{ item }">
            ...
            <div v-if="item.category_name" class="text-caption text-medium-emphasis">({{ item.category_name }})</div>
          </template>
          ...
          <template #no-data>
            <div class="text-center py-8 text-medium-emphasis">
              ...
              <div>No non-travel memos found for the selected filters.</div>
            </div>
          </template>
        </v-data-table>
      </v-card-text>
    ...
    if (window.ApmVuetifyPage) {
        window.ApmVuetifyPage.bind(MOUNT_ID, bootNonTravelIndex);
    }
})();
