...
class Tasks_mdl extends CI_Model {
    ...
    public function add_activity($data) {
        return $this->db->insert('planner_tasks', $data);
    }
    ...
    public function get_reports($staff_id) {
        $this->db->select('reports.*, planner_tasks.activity_name');
        $this->db->from('reports');
        $this->db->join('planner_tasks', 'planner_tasks.activity_id = reports.activity_id');
        $this->db->where('planner_tasks.staff_id', $staff_id);
    ...
    public function get_planner_tasks($staff_id = null, $output_id = null, $start_date = null, $end_date = null, $limit = null, $offset = null) {
        $this->db->select('
            planner_tasks.activity_id, 
            planner_tasks.activity_name, 
            planner_tasks.workplan_id, 
            divisions.division_head,
            workplan_tasks.activity_name AS work_activity_name, 
            planner_tasks.start_date, 
            planner_tasks.end_date, 
            reports.report_id,
            reports.status as report_status,
            reports.description as report,
            reports.report_date,
            planner_tasks.comments, 
            planner_tasks.staff_id, 
            planner_tasks.status,
            DATEDIFF(planner_tasks.end_date, planner_tasks.start_date)+1 AS activity_days
        ');
        $this->db->from('planner_tasks');
        
        // Apply filters if provided
        if (!empty($output_id)) {
            $this->db->where('planner_tasks.workplan_id', $output_id);
        }
        if (!empty($start_date)) {
            $this->db->where('planner_tasks.start_date >=', $start_date);
        }
        if (!empty($end_date)) {
            $this->db->where('planner_tasks.end_date <=', $end_date);
        }
        
        // Join the workplan_tasks and divisions tables
        $this->db->join('workplan_tasks', 'workplan_tasks.id = planner_tasks.workplan_id');
        $this->db->join('divisions', 'divisions.division_id = workplan_tasks.division_id');
    
        // Use a LEFT JOIN for reports to include all planner_tasks
        $this->db->join('reports', 'planner_tasks.activity_id = reports.activity_id', 'left');
    
        // Apply limit and offset for pagination
        if ($limit !== null) {
            $this->db->limit($limit, $offset);
        }
        $this->db->order_by('planner_tasks.end_date','ASC');
        $query = $this->db->get();
        return $query->result();
    }
    
    public function get_pending_planner_tasks($staff_id=null, $output_id = null, $start_date = null, $end_date = null,$limit = null, $offset = null) {
    ...
    public function get_planner_tasks_count($output_id = null, $start_date = null, $end_date = null) {
    ...
    public function get_reports_full(
        ...
        $this->db->select('
            ...
            planner_tasks.comments, 
            planner_tasks.staff_id, 
            planner_tasks.status AS activity_status,
            DATEDIFF(planner_tasks.end_date, planner_tasks.start_date)+1 AS activity_days,
            reports.report_id,
            reports.report_date,
            reports.week,
        ...
        $this->db->join('staff', 'staff.staff_id = planner_tasks.staff_id', 'left');
        // Join other related tables
        $this->db->join('workplan_tasks', 'workplan_tasks.id = planner_tasks.workplan_id');
        $this->db->join('divisions', 'workplan_tasks.division_id = divisions.division_id');
        $this->db->join('reports', 'reports.activity_id = planner_tasks.activity_id', 'left');
        ...
        if (!empty($activity_name)) {
            $this->db->like('planner_tasks.activity_name', $activity_name);
        }
