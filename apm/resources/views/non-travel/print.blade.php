...
<head>
    ...
    <style>
        ...
        .footer .line { border-top: 1px solid #999; margin: 6px 14mm 4px; }
...
<body>
    ...
    <div class="title">
        <h2>Non-Travel Memo</h2>
        <div class="small muted">Reference: #{{ $nonTravel->id }}</div>
    </div>
    ...
    <div class="section">
        <table class="no-border">
            <tr>
                ...
                <td class="w-50">{{ $nonTravel->staff->full_name ?? ($nonTravel->staff->name ?? 'N/A') }}</td>
                <td class="w-25"><strong>Category</strong></td>
                <td>{{ $nonTravel->nonTravelMemoCategory->name ?? 'N/A' }}</td>
            </tr>
            <tr>
    ...
    <div class="section">
        <h3 class="mb-8">Request Details</h3>
        <table>
            <tr>
            ...
            <tr>
                <th>Background</th>
                <td colspan="3">{!! \App\Helpers\PrintHelper::sanitizeRichTextForMpdf($nonTravel->background ?? '') !!}</td>
            </tr>
            <tr>
                <th>Description / Justification</th>
                <td colspan="3">{!! \App\Helpers\PrintHelper::sanitizeRichTextForMpdf($nonTravel->justification ?? $nonTravel->activity_request_remarks ?? '') !!}</td>
            </tr>
        ...
        <table>
            ...
            <tbody>
                ...
                @foreach (($fundCodes ?? []) as $code)
                    @php
                        $items = [];
                        if (is_array($breakdown ?? null)) {
                            $items = $breakdown[$code->id] ?? [];
                        }
                    @endphp
                    @if (!empty($items))
                        <tr>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tbody>
                @php 
                    $hasA = false; 
                    $files = is_array($attachments) ? $attachments : (is_string($attachments) ? json_decode($attachments, true) : []);
