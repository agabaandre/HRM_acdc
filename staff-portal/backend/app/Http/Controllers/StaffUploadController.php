...
class StaffUploadController extends Controller
{
    ...
    public function signature(string $filename): BinaryFileResponse
    {
    ...
    public function passport(string $filename): BinaryFileResponse
    {
        $safe = basename(str_replace('\\', '/', $filename));
        $path = StaffStorage::ciPath('staff/passport_biodata/'.$safe);
        if (! is_file($path)) {
            abort(404);
        }
