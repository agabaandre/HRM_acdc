...
onMounted(async () => {
  ...
  if (!token) {
    notifyError('Missing confirmation token.')
    busy.value = false
    return
  }
  try {
    ...
    notifyError('This link is invalid or was already used.')
  } finally {
    busy.value = false
  }
})
...
<template>
  <div>
    ...
    <div class="cbp-card">
      ...
      <p v-else class="muted">This link is invalid or was already used.</p>
      <p class="back">
