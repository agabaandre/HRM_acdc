            ...
            <form action="{{ request('change_request') ? route('change-requests.store') : ($activity->is_single_memo ? route('activities.single-memos.update', [$matrix, $activity]) : route('matrices.activities.update', [$matrix, $activity])) }}" method="POST" id="activityForm" enctype="multipart/form-data">
                ...
                @if(request('change_request'))
                    @include('change-requests.partials.travel-cash-and-details', [
                        ...
                        'supportingReasonsPlaceholder' => 'Please explain why you are requesting changes to this activity...',
                    ])
                @endif
...
$(document).ready(function () {
    ...
    // Budget codes loading and management (extramural: one code; intramural: up to 2)
            ...
            if (data.length) {
                ...
                data.forEach(code => {
                    ...
                    budgetCodesSelect.append(
                        `<option value="${code.id}" data-balance="${code.budget_balance}" data-current-activity-budget="${currentActivityBudget}" data-funder-id="${code.funder_id || ''}" data-show-activity-code="${code.show_activity_code ? 1 : 0}" data-activity-code-label="${(code.activity_code_label || 'Activity Code *').replace(/"/g, '&quot;')}">${label}</option>`
                    );
                });
        ...
        $('.budget-body').each(function () {
            ...
            if (isChangeRequest) {
                ...
                if (newItemsTotal > 0) {
                    ...
                    // budgetBalance is the current balance (excluding this activity's budget)
    ...
    function initializeExistingData() {
        ...
        const fundTypeText = $('#fund_type option:selected').text().toLowerCase();
        if (fundTypeText.indexOf("intramural") > -1) {
