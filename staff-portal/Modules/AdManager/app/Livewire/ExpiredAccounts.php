...
class ExpiredAccounts extends Component
{
    ...
    {
        $paginator = $service->paginateAccountsToDisable($this->search, $this->perPage, $this->getPage());
        $range = $this->tableRange($paginator);

        return view('admanager::livewire.expired-accounts', [
            'staff' => $paginator,
            'from' => $range['from'],
            'to' => $range['to'],
            'total' => $range['total'],
