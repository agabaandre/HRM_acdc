...
use App\Http\Controllers\Concerns\SpecialMemoListIndexResponses;
...
{
    use SpecialMemoListIndexResponses;
    use SendsSelfDocumentPdfEmail;

    public function index(Request $request): View|JsonResponse
    {
        if (\App\Support\ApmListFragment::wants($request)) {
            return $this->getSpecialMemosIndexAjax($request);
        }

        return view('special-memo.index', [
            'pageConfig' => $this->buildSpecialMemosIndexPageConfig($request),
        ]);
    }
    ...
    public function store(Request $request): RedirectResponse|\Illuminate\Http\JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'cash_carrier_staff_ids' => 'nullable|array',
        ...
        if ($request->boolean('request_travel_with_cash')) {
            $request->validate(\App\Support\TravelCashCarriers::cashCarrierValidationRules());
        }
