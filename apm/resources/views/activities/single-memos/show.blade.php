...
@section('content')
    <div class="min-h-screen bg-gray-50">
            ...
            <div class="container-fluid">
                <div class="py-4">
                    ...
                    <div class="d-flex gap-2 justify-content-end align-items-center" style="flex-wrap: nowrap !important; white-space: nowrap !important; overflow-x: auto; width: 100%;">
                        <a wire:navigate href="{{ route('activities.single-memos.index') }}" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-1" style="flex-shrink: 0;">
                    ...
                    @endif
                        ...
                        @php
                            ...
                            $canUpdateOwners = can_manage_memo_owners_for_division((int) ($activity->division_id ?? 0));
                        @endphp
                        ...
                        @elseif($isAdmin && ($activity->overall_status ?? '') === 'archived')
                            <button type="button" class="btn btn-outline-success btn-sm d-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#unarchiveSingleMemoModal" style="flex-shrink: 0;">
                    ...
                    @endif
                        ...
                        @if ($activity->overall_status === 'approved')
                            ...
                            @include('partials.email-pdf-modal', [
                                'emailPdfFormAction' => route('matrices.activities.email-pdf', [$activity->matrix, $activity]),
        ...
        @if($isAdmin && ($activity->overall_status ?? '') === 'archived')
            <div class="modal fade" id="unarchiveSingleMemoModal" tabindex="-1" aria-labelledby="unarchiveSingleMemoModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="unarchiveSingleMemoModalLabel">Unarchive Single Memo</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <form method="POST" action="{{ route('activities.single-memos.unarchive', $activity) }}">
                                @csrf
                ...
                <div class="container-fluid py-4"> <!-- Reopen container-fluid -->
                   ...
                   <div class="col-lg-12">
                    ...
                    <div class="card shadow-sm mb-4">
                        ...
                        @php
                            $isReturnedAtHodLevel = isdivision_head($activity)
                                && $activity->overall_status == 'returned'
                                && (int) $activity->approval_level === 1;
                            $canShowApprovalActions = can_take_action_generic($activity)
                                || (is_with_creator_generic($activity) && !in_array($activity->overall_status, ['draft', 'returned'], true))
                                || (isdivision_head($activity) && $activity->overall_status == 'pending' && (int) $activity->approval_level === 1);
                        @endphp
                        @if ($isReturnedAtHodLevel || ($activity->overall_status !== 'returned' && $canShowApprovalActions))
                        ...
                        @if($activity->overall_status === 'draft' && can_edit_memo($activity))
    ...
    @if (can_convert_returned_memo_to_non_travel($activity))
        @include('partials.convert-to-non-travel-modal', [
            'modalId' => 'convertSingleMemoToNonTravelModal',
            'formAction' => route('activities.single-memos.convert-to-non-travel', $activity),
        ])
    @endif
