...
Route::prefix('apm/v1')->group(function () {
    ...
    Route::middleware(['auth:api', 'apm.api.context'])->group(function () {
        ...
        Route::get('documents/{type}/{status}', [ApmDocumentController::class, 'listByTypeAndStatus'])->where('type', 'special_memo|matrix|activity|non_travel_memo|service_request|arf|change_request|other_memo')->where('status', 'pending|approved|draft|rejected|returned');
        Route::get('documents/{type}/{id}', [ApmDocumentController::class, 'show'])->where('type', 'special_memo|matrix|activity|non_travel_memo|service_request|arf|change_request|other_memo')->where('id', '[0-9]+');
        Route::get('documents/attachments/{type}/{id}/{index}', [ApmDocumentController::class, 'attachment'])
