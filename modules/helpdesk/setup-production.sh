...
helpdesk_validate_staff_api_env "$BACKEND_ENV" "$STAFF_ROOT" || die "Staff Share API credentials are required. Set STAFF_API_* in $STAFF_ROOT/modules/apm/.env then re-run ./setup-production.sh"
