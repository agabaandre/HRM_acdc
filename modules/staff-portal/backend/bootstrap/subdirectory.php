...
/**
 * Strip the Apache mount path (e.g. /staff/backend) from REQUEST_URI so Laravel
 * routes like "/auth/spa-bridge" match, while SCRIPT_NAME keeps the public prefix
 * for URL generation (see App\Support\RuntimeUrl).
 */
...
if ($basePath === '' && isset($_SERVER['REQUEST_URI'])
    && preg_match('#^(/[^/]+/backend)(?:/|$)#', (string) ($_SERVER['REQUEST_URI'] ?? ''), $m) === 1) {
    $basePath = $m[1];
}
