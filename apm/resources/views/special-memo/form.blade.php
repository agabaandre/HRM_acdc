<div class="card border-0 shadow-sm mb-5">
    ...
    <div class="card-body">
        ...
        <div class="col-md-6">
                ...
                <input type="text" name="activity_title" id="activity_title" class="form-control " value="{{ old('activity_title', $specialMemo->activity_title ?? '') }}" required>
            </div>
            <div class="col-md-6">
                ...
                <select name="request_type_id" id="request_type_id" class="form-select " required>
                    ...
                    <option value="{{ $type->id }}" {{ old('request_type_id', $specialMemo->request_type_id ?? '') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-6">
                ...
                <select name="responsible_person_id" id="responsible_person_id" class="form-select select2 " required>
                    ...
                    <option value="{{ $member->staff_id }}" {{ old('responsible_person_id', $specialMemo->responsible_person_id ?? '') == $member->staff_id ? 'selected' : '' }}>{{ $member->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="date_from" id="date_from" class="form-control datepicker " value="{{ old('date_from', $specialMemo->date_from ? $specialMemo->date_from->format('Y-m-d') : '') }}" required>
            </div>
            ...
            <div class="col-md-3">
                ...
                <input type="text" name="date_to" id="date_to" class="form-control datepicker " value="{{ old('date_to', $specialMemo->date_to ? $specialMemo->date_to->format('Y-m-d') : '') }}" required>
            </div>
            <div class="col-md-4">
                ...
                <select name="location_id[]" id="location_id" class="form-select border-success" multiple required>
                    @foreach($locations as $location)
                        <option value="{{ $location->id }}" {{ in_array($location->id, old('location_id', json_decode($specialMemo->location_id ?? '[]', true) ?? [])) ? 'selected' : '' }}>
                            {{ $location->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select name="internal_participants[]" id="internal_participants" class="form-select border-success" multiple required>
                    @foreach($staff as $member)
                        <option value="{{ $member->staff_id }}" {{ in_array($member->staff_id, old('internal_participants', array_keys(json_decode($specialMemo->internal_participants ?? '{}', true) ?? []))) ? 'selected' : '' }}>
                            {{ $member->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="total_external_participants" id="total_external_participants" class="form-control border-success" value="{{ old('total_external_participants', $specialMemo->total_external_participants ?? 0) }}" min="1" required>
            </div>
              <div class="col-md-12">
