        ...
        <div class="card-body p-4">
            <form action="{{ route('service-requests.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <input type="hidden" name="responsible_person_id"
                    value="{{ $sourceData ? $sourceData->staff_id : (auth()->check() ? auth()->user()->staff_id : 1) }}">
                ...
                <input type="hidden" name="service_title"
                    value="{{ $sourceData ? $sourceData->title ?? $sourceData->activity_title : 'Service Request' }}">
                <input type="hidden" name="description"
                    value="{{ $sourceData ? $sourceData->description ?? $sourceData->background : 'Service Request Description' }}">
                <input type="hidden" name="justification"
                    value="{{ $sourceData ? $sourceData->justification ?? $sourceData->activity_request_remarks : 'Service Request Justification' }}">
                                    ...
                                    @foreach ($costItems as $index => $costItem)
                                        <td>
                                            <input type="number"
                                                ...
                                                step="0.01" data-cost-item="{{ $costItem->name }}"
                                ...
                                @if ($otherCostItems->isNotEmpty())
                                    @foreach ($otherCostItems as $index => $costItem)
                                        <tr>
                                            ...
                                            <td>
                                                <input type="number" name="other_costs[{{ $index }}][unit_cost]"
                                                    class="form-control border-success cost-input" value="0"
                                                    step="0.01" placeholder="Enter unit cost">
                                            </td>
                                            <td>
                        ...
                        <div class="col-md-4">
                            <div class="card border-success">
                                <div class="card-body text-center">
                                    ...
                                    <h4 class="text-success" id="originalBudgetAmount">
                                        ${{ number_format($totalOriginal, 2) }}</h4>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
