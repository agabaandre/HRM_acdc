...
async function sendQuestion(text: string): Promise<void> {
  ...
  try {
    ...
    notifyError(apiErrorMessage(e, 'Ask Service Desk could not respond. Try again or log a ticket.'))
  } finally {
    sending.value = false
    await scrollToBottom()
  }
...
<template>
  <div>
    ...
    <CbpPageHeading title="Ask Service Desk">
      <template #lede>
    ...
    <div class="hd-ask-layout">
      <v-card class="hd-ask-panel" variant="outlined" aria-label="Ask Service Desk conversation">
        <v-card-item class="hd-ask-header">
          ...
          <v-card-title class="hd-ask-header-title">Service Desk assistant</v-card-title>
        ...
        <div ref="messagesEl" class="hd-ask-messages">
          ...
          >
            ...
            <v-sheet
              ...
              rounded="lg"
            >
              <span class="hd-bubble-label">{{ m.role === 'user' ? 'You' : 'Service Desk assistant' }}</span>
      ...
      <aside class="hd-ask-side" aria-label="Ask Service Desk tips">
