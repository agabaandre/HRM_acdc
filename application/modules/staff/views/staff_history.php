<style>
	...
	/* Hide original export buttons in filters after they're moved */
...
<?= form_open_multipart(base_url('staff/staff_history'), ['id' => 'staff_form', 'class' => 'staff', 'method' => 'get']) ?>

<div class="alert alert-secondary small mb-3">
	<strong>Staff History</strong> lists everyone who had at least one <code>staff_contracts</code> row overlapping the report period (including separated staff).
	If several contracts overlap, the row shows the contract with the <strong>largest overlap</strong> with that period (tie-break: most recent <code>staff_contract_id</code>).
</div>
		...
		<table class="table table-striped table-bordered">
			<thead>
				<tr>
					...
					<th>Contract start (filtered)</th>
					<th>Contract end (filtered)</th>
...
$(document).ready(function() {
	...
	$(document).ready(function() {
		...
		if (exportButtons.length && exportButtons.html().trim() !== '') {
			...
			var csvUrl = '<?php echo base_url(); ?>staff/staff_history/1' + queryString;
			var pdfUrl = '<?php echo base_url(); ?>staff/staff_history/0/1' + queryString;
	...
	function updateExportLinks() {
		...
		var baseUrl = '<?php echo base_url(); ?>staff/staff_history';
		
		// Update export links in top right (visible buttons)
		$('#exportButtonsTop a').each(function() {
			var href = $(this).attr('href');
			if (href && href.includes('staff_history')) {
		...
		$('.export-buttons a[href*="staff_history"]').each(function() {
			var href = $(this).attr('href');
			// Check for PDF first (before CSV, since /0/1 contains /1)
	...
	function loadStaffHistoryData() {
		...
		$.ajax({
			url: '<?php echo base_url(); ?>staff/get_staff_history_data_ajax',
			method: 'POST',
			data: postData,
			dataType: 'json',
			success: function(response) {
				if (response.html) {
					...
					$('#staffTableBody').html('<tr><td colspan="25" class="text-center">No data available</td></tr>');
				}
			},
			error: function(xhr, status, error) {
				console.error('AJAX Error:', status, error);
