...
public function users()
{
	if ($this->api_login()) {
		try {
			...
			$this->db->select("user.user_id, user.password, COALESCE(NULLIF(TRIM(CONCAT(COALESCE(staff.fname, ''), ' ', COALESCE(staff.lname, ''))), ''), user.name) AS name, user.role, user.auth_staff_id, user.status, user.created_at, user.changed, user.isChanged, user.photo, user.signature, user.is_approved, user.is_verfied, user.langauge, user.allow_email_login, staff.work_email AS email");
			$this->db->from('user');
