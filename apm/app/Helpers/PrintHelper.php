...
class PrintHelper
{
    ...
    /**
     * Normalize stored Summernote / Quill HTML for edit forms (Quill initial value).
     *
     * Some legacy rows store entity-encoded HTML; show/PDF paths decode via sanitizeRichTextForMpdf
     * but the editor must receive real markup or Quill renders an empty field.
     */
    public static function prepareRichTextForEditor(?string $html): string
    {
        ...
        if (preg_match('/^<div[^>]*class="[^"]*note-editable[^"]*"[^>]*>(.*)<\/div>\s*$/is', $html, $matches)) {
            $html = trim((string) ($matches[1] ?? ''));
        }

        if ($html !== '' && ! preg_match('/<(p|div|ul|ol|table|h[1-6]|blockquote|pre|img|br)\b/i', $html)) {
            $html = '<p>'.htmlspecialchars($html, ENT_QUOTES | ENT_HTML5, 'UTF-8').'</p>';
        }
