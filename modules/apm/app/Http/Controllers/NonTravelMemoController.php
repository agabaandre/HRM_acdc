...
use App\Http\Controllers\Concerns\NonTravelMemoListIndexResponses;
use Illuminate\Http\JsonResponse;

class NonTravelMemoController extends Controller
{
    use NonTravelMemoListIndexResponses;
    ...
    public function index(Request $request): View|JsonResponse
    {
        if (\App\Support\ApmListFragment::wants($request)) {
            return $this->getNonTravelMemosIndexAjax($request);
        }

        return view('non-travel.index', [
            'pageConfig' => $this->buildNonTravelMemosIndexPageConfig($request),
        ]);
    }
   ...
   public function create(): View
    ...
    public function update(Request $request, NonTravelMemo $nonTravel): RedirectResponse|\Illuminate\Http\JsonResponse
    {
       ...
       $attachments = ChangeRequest::collectAttachmentsFromRequest($request, $existingAttachments, 'uploads/non-travel');
        ...
        $ntUpdate = [
            ...
            'workplan_activity_code' => clean_unicode((string) ($data['activity_code'] ?? '')),
            'activity_request_remarks' => clean_unicode($data['activity_request_remarks']),
            'background' => clean_unicode($data['background']),
            'justification' => clean_unicode($data['justification']),
            'budget_breakdown' => $budgetBreakdownJson,
    ...
    public function resubmit(Request $request, NonTravelMemo $nonTravel): RedirectResponse
    {
        ...
        // Resubmit only after an approver has returned the memo (not while status is still pending)
        if ($nonTravel->overall_status !== 'returned') {
