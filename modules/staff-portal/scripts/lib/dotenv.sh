...
# True when a .env value is considered configured (non-empty, not a placeholder).
...
# Set key only when the target .env key is missing, or on first-time .env creation.
