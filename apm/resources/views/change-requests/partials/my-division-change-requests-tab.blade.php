<div class="table-responsive">
    <table class="table table-hover mb-0">
        ...
        <tbody>
            @forelse($myDivisionChangeRequests as $index => $changeRequest)
                <tr>
                    <td class="text-center fw-bold">{{ $myDivisionChangeRequests->firstItem() + $index }}</td>
...
@if($myDivisionChangeRequests->hasPages())
    <div class="card-footer bg-white border-top-0">
        <div class="d-flex justify-content-between align-items-center">
            <div class="text-muted">
                Showing {{ $myDivisionChangeRequests->firstItem() }} to {{ $myDivisionChangeRequests->lastItem() }} of {{ $myDivisionChangeRequests->total() }} results
            </div>
            <div>
                {{ $myDivisionChangeRequests->links() }}
            </div>
        </div>
    </div>
@endif
