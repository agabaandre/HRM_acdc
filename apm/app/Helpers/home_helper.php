...
if (! function_exists('get_staff_pending_action_count')) {
    ...
    function get_staff_pending_action_count(string $module, ?int $staffId = null): int
    {
        ...
        switch ($module) {
            ...
            case 'other-memo':
                return get_pending_other_memo_count($staffId);
            default:
                return 0;
        }
    }
...
if (! function_exists('get_pending_other_memo_count')) {
    /**
     * Get count of pending other memos awaiting action from the current staff member (matches PendingApprovalsService).
     */
    function get_pending_other_memo_count(int $staffId): int
    {
        try {
            $pendingApprovalsService = new \App\Services\PendingApprovalsService([
                'staff_id' => $staffId,
            ...
            return $summaryStats['by_category']['Other Memo'] ?? 0;
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Error getting pending other memo count: '.$e->getMessage());
...
if (! function_exists('generate_pdf')) {
     ...
     *                          - 'attachments_appendix_title' (string): Optional appendix heading (default "Appendix — Attachments")
    ...
    function generate_pdf($view, $data = [], $options = [])
    {
        ...
        if (is_array($appendixAttachments) && $appendixAttachments !== []) {
            $appendixTitle = $options['attachments_appendix_title'] ?? 'Appendix — Attachments';
            if (! is_string($appendixTitle) || trim($appendixTitle) === '') {
                $appendixTitle = 'Appendix — Attachments';
            }
            \App\Helpers\PrintHelper::appendAttachmentsAppendixToMpdf($mpdf, $appendixAttachments, $appendixTitle);
        }
...
if (! function_exists('get_staff_total_pending_count')) {
    ...
    function get_staff_total_pending_count(?int $staffId = null): int
    {
        ...
        $modules = ['matrices', 'non-travel', 'special-memo', 'other-memo', 'service-requests', 'request-arf', 'single-memo', 'change-request'];
        $total = 0;
...
if (! function_exists('get_my_returned_other_memo_count')) {
    /**
     * Count of other memos returned to the creator for revision (matches ReturnedMemosService::getReturnedOtherMemos).
     *
     * @param  int  $staffId  Creator staff_id
     */
    function get_my_returned_other_memo_count(int $staffId): int
    {
        try {
            return \App\Models\OtherMemo::query()
                ->where('overall_status', \App\Models\OtherMemo::STATUS_RETURNED)
                ->where('staff_id', $staffId)
                ->count();
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Error getting returned other memos count: '.$e->getMessage());
