...
<div class="card shadow-sm border-0">
    ...
    <div class="card-body">
                            ...
                            @if(!$crHideBudgetForViewer && $changeRequest->has_budget_breakdown_changed)
                                <div class="col-md-12 mb-3">
                                    <div class="border rounded p-3">
                                        ...
                                        @include('change-requests.partials.budget-breakdown-compare', [
        ...
        @endif
                ...
                @php
                    $isReturnedAtHodLevel = isdivision_head($changeRequest)
                        && $changeRequest->overall_status == 'returned'
                        && (int) $changeRequest->approval_level === 1;
                    $canShowApprovalActions = can_take_action_generic($changeRequest)
                        || (is_with_creator_generic($changeRequest) && !in_array($changeRequest->overall_status, ['draft', 'returned'], true));
                @endphp
                @if($isReturnedAtHodLevel || ($changeRequest->overall_status !== 'returned' && $canShowApprovalActions))
