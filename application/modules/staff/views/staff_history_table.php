<?php
$i = 1;
$offset = ($page * $per_page);
if ($offset != '') {
	$i = $offset + 1;
}

if (empty($staffs)) : ?>
	<tr>
		<td colspan="26" class="text-center">No staff had a contract overlapping the selected period.</td>
	</tr>
<?php else :
	foreach ($staffs as $data) :
		?>
	<tr>
		<td><?= $i++ ?></td>
		<td><?= $data->SAPNO ?></td>
		<td><?= $data->title ?></td>
		<td>
			<?php
			$surname = $data->lname;
			$other_name = $data->fname;
			$image_path = staff_secure_upload_url('photo', $data->photo ?? '');
			echo generate_user_avatar($surname, $other_name, $image_path, $data->photo);
			?>
		</td>
		<td><a href="#" class="view-staff-profile" data-staff-id="<?php echo $data->staff_id; ?>" data-bs-toggle="modal" data-bs-target="#employeeProfileModal"><?= $data->lname . ' ' . $data->fname . ' ' . @$data->oname ?></a></td>
		<td><?= $data->gender ?></td>
		<td><?= !empty($data->date_of_birth) ? $data->date_of_birth : 'N/A' ?></td>
		<td><?php
			if (!empty($data->date_of_birth)) {
				echo calculate_age($data->date_of_birth);
			} else {
				echo 'N/A';
			}
		?></td>
		<td><?= $data->nationality; ?></td>
		<td><?= !empty($data->region_name) ? html_escape($data->region_name) : 'N/A'; ?></td>
		<td><?= @$data->duty_station_name; ?></td>
		<td><?= @$data->division_name; ?></td>
		<td><?= @$data->grade; ?></td>
		<td><?= @character_limiter($data->job_name, 30); ?></td>
		<td><?= !empty($data->status) ? $data->status : 'N/A'; ?></td>
		<td><?= !empty($data->initiation_date) ? $data->initiation_date : 'N/A'; ?></td>
		<td><?= !empty($data->start_date) ? $data->start_date : 'N/A'; ?></td>
		<td><?= !empty($data->end_date) && $data->end_date !== '0000-00-00' ? $data->end_date : 'Open / N/A'; ?></td>
		<td><?= !empty($data->initiation_date) ? years_of_tenure($data->initiation_date) : 'N/A'; ?></td>
		<td><?= @character_limiter($data->job_acting, 30); ?></td>
		<td><?= @staff_name($data->first_supervisor); ?></td>
		<td><?= @staff_name($data->second_supervisor); ?></td>
		<td><?= @$data->funder; ?></td>
		<td><?= $data->work_email; ?></td>
		<td><?= @$data->tel_1 ?> <?php if (!empty($data->tel_2)) {
			echo '  ' . $data->tel_2;
		} ?></td>
		<td><?= $data->whatsapp ?></td>
	</tr>
<?php endforeach;
endif; ?>
