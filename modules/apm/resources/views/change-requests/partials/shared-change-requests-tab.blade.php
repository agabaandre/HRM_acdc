<div class="table-responsive">
    <table class="table table-hover mb-0">
        ...
        <tbody>
            @forelse($sharedChangeRequests as $index => $changeRequest)
                <tr>
                    <td class="text-center fw-bold">{{ $sharedChangeRequests->firstItem() + $index }}</td>
                    <td class="text-center">
                        @include('change-requests.partials.list-document-cell', ['changeRequest' => $changeRequest])
                    </td>
...
@if($sharedChangeRequests->hasPages())
    <div class="card-footer bg-white border-top-0">
        <div class="d-flex justify-content-between align-items-center">
            <div class="text-muted">
                Showing {{ $sharedChangeRequests->firstItem() }} to {{ $sharedChangeRequests->lastItem() }} of {{ $sharedChangeRequests->total() }} results
            </div>
            <div>
                {{ $sharedChangeRequests->links() }}
            </div>
        </div>
    </div>
@endif
