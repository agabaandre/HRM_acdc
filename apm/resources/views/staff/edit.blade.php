                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" 
                                       ...
                                       id="middlename" 
                                       name="middlename" 
                                       value="{{ old('middlename', $staff->middlename) }}" 
                    ...
                    <div class="row g-3 mb-4">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select @error('division_id') is-invalid @enderror" id="division_id" name="division_id" required>
                                    ...
                                    @foreach($divisions as $division)
                                        <option value="{{ $division->id }}" {{ old('division_id', $staff->division_id) == $division->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-select @error('directorate_id') is-invalid @enderror" id="directorate_id" name="directorate_id" required>
                                    <option value="">Select Directorate</option>
                                    @foreach($directorates as $directorate)
                                        <option value="{{ $directorate->id }}" {{ old('directorate_id', $staff->directorate_id) == $directorate->id ? 'selected' : '' }}>
                                            {{ $directorate->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-select @error('duty_station_id') is-invalid @enderror" id="duty_station_id" name="duty_station_id" required>
                                    <option value="">Select Duty Station</option>
                                    @foreach($dutyStations as $dutyStation)
                                        <option value="{{ $dutyStation->id }}" {{ old('duty_station_id', $staff->duty_station_id) == $dutyStation->id ? 'selected' : '' }}>
                                            {{ $dutyStation->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-select @error('employment_status') is-invalid @enderror" id="employment_status" name="employment_status" required>
                                    ...
                                    <option value="Full-time" {{ old('employment_status', $staff->employment_status) == 'Full-time' ? 'selected' : '' }}>Full-time</option>
                                    <option value="Part-time" {{ old('employment_status', $staff->employment_status) == 'Part-time' ? 'selected' : '' }}>Part-time</option>
                                    <option value="Contract" {{ old('employment_status', $staff->employment_status) == 'Contract' ? 'selected' : '' }}>Contract</option>
                                    <option value="Consultant" {{ old('employment_status', $staff->employment_status) == 'Consultant' ? 'selected' : '' }}>Consultant</option>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select @error('supervisor_id') is-invalid @enderror" id="supervisor_id" name="supervisor_id">
                                    ...
                                    @foreach($supervisors as $supervisor)
                                        <option value="{{ $supervisor->id }}" {{ old('supervisor_id', $staff->supervisor_id) == $supervisor->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('remarks') is-invalid @enderror" 
                                      ...
                                      name="remarks" 
                                      rows="3" 
                                      placeholder="Enter additional remarks">{{ old('remarks', $staff->remarks) }}</textarea>
                            @error('remarks')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="card-body p-4">
                    <div class="form-group mb-4">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="active" name="active" value="1" {{ old('active', $staff->active) == '1' ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="border rounded p-2 text-center">
                            <img src="{{ asset('storage/' . $staff->profile_photo) }}" alt="Profile Photo" class="img-fluid rounded" style="max-height: 150px;">
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input type="file" 
                               ...
                               name="profile_photo"
                               accept="image/*">
                        <small class="text-muted">Leave empty to keep current photo. Supported formats: JPG, PNG, GIF (Max: 2MB)</small>
                        @error('profile_photo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card-body p-4">
                    <div class="form-group mb-4">
                        ...
                        <select class="form-select @error('access_level') is-invalid @enderror" id="access_level" name="access_level">
                            ...
                            <option value="1" {{ old('access_level', $staff->access_level) == '1' ? 'selected' : '' }}>Basic User</option>
                            <option value="2" {{ old('access_level', $staff->access_level) == '2' ? 'selected' : '' }}>Advanced User</option>
                            <option value="3" {{ old('access_level', $staff->access_level) == '3' ? 'selected' : '' }}>Administrator</option>
                        </select>
