...
- **HoD / contributor deadline reminders**: always at **HoD reminder time** on the calendar day **one day before** the submission deadline and on **deadline day** (`hod_reminder_days_before_deadline` always includes offsets **1** and **0** when saved; extra offsets such as **2** are optional). The scheduler matches **calendar day** and **H:i**; sends stop after the deadline passes.
...
- Compiled / reminder behaviour is implemented in `WeeklyBriefingCompiledSummaryCommand`, `WeeklyBriefingHodRemindersCommand`, and `WeeklyBriefingDirectorReviewRemindersCommand` (see below). Text reminders use `WeeklyBriefingNotificationMailer` → `SendNotificationEmailJob::dispatchSync` (same Exchange/SMTP path as approval alerts). **Compiled PDF attachments** use `sendEmail()` → Graph directly (binary attachments cannot be queued in the job JSON).
